...
rule download_genome_annot:
  output:
    fasta = temp("data/alignment_references/{species}_genome_{suffix}/genome.fasta"),
    gtf = temp("data/alignment_references/{species}_genome_{suffix}/genome.gtf")
  params:
  ...
  log: "data/alignment_references/{species}_genome_{suffix}/logs/download_genome_annot.log"
...
rule create_tapseq_annot:
  input:
    lambda wildcards: config["create_tapseq_ref"]["target_genes"]
      [wildcards.species + "_tapseq_" + wildcards.suffix]
  output:
    fasta = temp("data/alignment_references/{species}_tapseq_{suffix}/genome.fasta"),
    gtf = temp("data/alignment_references/{species}_tapseq_{suffix}/genome.gtf")
  params:
    BSgenome = lambda wildcards: config["create_tapseq_ref"]["BSgenome_id"][wildcards.species]
  log: "data/alignment_references/{species}_tapseq_{suffix}/logs/create_tapseq_ref.log"
